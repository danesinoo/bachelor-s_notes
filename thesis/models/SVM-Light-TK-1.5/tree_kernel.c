...
// ERRORS remove from norm Evaluate_norm the weight of tree kernels
...
double SIGMA;
double MU;
double REMOVE_LEAVES =
    0; // if equal to MU*LAMBDA2, it removes the leaves contribution;

short TKGENERALITY; // store the generality of the kernel PT is 2 whereas SST
                    // and ST is 1
                    //  used to load the opportune data structures (with SST and
                    //  ST uses faster approach)
...
void determine_sub_lists(
    FOREST *a, FOREST *b, nodePair *intersect,
    int *n); // pre-selection of pairs for SST and ST kernels
double Delta_SST_ST(TreeNode *Nx,
                    TreeNode *Nz); // delta function SST and ST kernels
double Delta_SK(TreeNode **Sx, TreeNode **Sz, int n,
                int m);                      // delta over children
double Delta_PT(TreeNode *Nx, TreeNode *Nz); // Delta for PT

void evaluateNorma(KERNEL_PARM *kernel_parm,
                   DOC *d); // evaluate norm of trees and vectors

double basic_kernel(KERNEL_PARM *kernel_parm, DOC *a, DOC *b, int i,
                    int j); // svm-light kernels
double tree_kernel(KERNEL_PARM *kernel_parm, DOC *a, DOC *b, int i,
                   int j);                             // ST and SST kernels
...
double choose_second_kernel(KERNEL_PARM *kernel_parm, DOC *a,
                            DOC *b); // choose a standard kernel
double choose_tree_kernel(KERNEL_PARM *kernel_parm, DOC *a,
                          DOC *b); // choose a tree kernel
...
double two_vector_set(KERNEL_PARM *kernel_parm, DOC *a, DOC *b, int memberA,
                      int memberB); // all_vs_all vectorial kernel
...
double Question_Answer_Classification(KERNEL_PARM *kernel_parm, DOC *a, DOC *b);
double SRL_re_ranking_ACL2009(KERNEL_PARM *kernel_parm, DOC *a, DOC *b);
double Jeopardy_QC(KERNEL_PARM *kernel_parm, DOC *a, DOC *b);
double Jeopardy_QAR(KERNEL_PARM *kernel_parm, DOC *a, DOC *b);
...
double evaluate_SST_ST(nodePair *pairs, int n) {
  ...
  for (i = 0; i < n; i++) {
    // printf("Score for the pairs (%s ,
    // %s)",pairs[i].Nx->sName,pairs[i].Nz->sName);fflush(stdout);
    // printf("\ntree 1: "); writeTreeString(pairs[i].Nx); printf("\ntree 2: ");
    // writeTreeString(pairs[i].Nz); printf("\n"); fflush(stdout);
    //
...
//-------------------------------------------------------------------------------------------------------
// Kernel with slot used in ACL07 and ECIR07 to simulate a fast PT (within the
// first tree level)
//-------------------------------------------------------------------------------------------------------
...
double Delta_ACL07(TreeNode *Nx, TreeNode *Nz) {
  ...
  if (delta_matrix[Nx->nodeID][Nz->nodeID] >= 0) {
    ...
    // Alessandro
    if (strcmp(Nx->pChild[0]->sName, "null") == 0 ||
        strcmp(Nz->pChild[0]->sName, "null") == 0)
...
double evaluate_SST_ACL07(nodePair *pairs, int n) {
  ...
  for (i = 0; i < n; i++) {
    ...
    if (strcmp(pairs[i].Nx->sName, "BOX") != 0 &&
        strcmp(pairs[i].Nz->sName, "BOX") !=
            0) // to be removed as it is verified that is useless
    {
...
double Delta_SK(TreeNode **Sx, TreeNode **Sz, int n, int m) {
  ...
  for (l = 1; l < p; l++) {
    ...
    for (i = l; i <= n; i++)
      for (j = l; j <= m; j++) {
        DP[i][j] = DPS[i][j] + LAMBDA * DP[i - 1][j] + LAMBDA * DP[i][j - 1] -
                   LAMBDA2 * DP[i - 1][j - 1];
...
double Delta_PT(TreeNode *Nx, TreeNode *Nz) {
  ...
  if (strcmp(Nx->sName, Nz->sName) != 0)
    ...
    //              "); writeTreeString(Nz); printf("\n\n");

    sum = MU * (LAMBDA2 + Delta_SK(Nx->pChild, Nz->pChild, Nx->iNoOfChildren,
                                   Nz->iNoOfChildren));
    //             printf("\n (node1:%s node2:%s) ----------------------->
...
double evaluate_PT(nodePair *pairs, int n) {
  ...
  for (i = 0; i < n; i++) {
    // printf("\n------------------------------------------------\n");fflush(stdout);
    // printf("Considering pair (%s_%d ,
    // %s_%d):",pairs[i].Nx->sName,pairs[i].Nx->nodeID,pairs[i].Nz->sName,pairs[i].Nz->nodeID);fflush(stdout);
    // printf("\ntree 1: "); writeTreeString(pairs[i].Nx); printf("\ntree 2: ");
    // writeTreeString(pairs[i].Nz); printf("\n");fflush(stdout);
    contr = Delta_PT(pairs[i].Nx, pairs[i].Nz) -
            REMOVE_LEAVES; // remove the contribution of leaves
                           // printf("Score: %1.15f\n",contr); fflush(stdout);
    //  if(pairs[i].Nx->iNoOfChildren!=0 ||
    //  pairs[i].Nz->iNoOfChildren!=0)contr-=REMOVE_LEAVES;
      ...
      do {
        do {
          ...
          if (*n > MAX_NUMBER_OF_PAIRS) {
            printf("ERROR: The number of identical parse nodes exceed the "
                   "current capacityn\n");
            exit(-1);
          }
          delta_matrix[list_a[i].node->nodeID][list_b[j].node->nodeID] = -1.0;
          //  TEST           printf("Evaluating-Pair: (%s  ,  %s) i %d,j %d
          //  j_old%d
...
double string_kernel_not_norm(KERNEL_PARM *kernel_parm, DOC *a, DOC *b, int i,
                              int j) {
  if (a->num_of_trees == 0 || b->num_of_trees == 0)
    ...
    if (a->num_of_trees <= i)
      printf("\n     : position %d of the tree forest of the first instance "
             "(line %d)\n\n",
             i, a->docnum + 1);
    if (b->num_of_trees <= j)
      printf("\n     : position %d of the tree forest of the second instance "
             "(line %d)\n\n",
             j, b->docnum + 1);
    fflush(stdout);
    exit(-1);
    ...
    return SK(a->forest_vec[i]->root->pChild, b->forest_vec[j]->root->pChild,
...
double string_kernel(KERNEL_PARM *kernel_parm, DOC *a, DOC *b, int i, int j) {
  ...
  // printf("\ntree 2: "); writeTreeString(b->forest_vec[j]->root);
  // printf("\n"); fflush(stdout);
...
double tree_kernel_not_norm(KERNEL_PARM *kernel_parm, DOC *a, DOC *b, int i,
  ...
  if (a->forest_vec[i]->orderedNodeSet != NULL &&
    ...
    // if trees are not empty, from empty orderedNodeList => they are sequences
    // and should be run wtih kernel 6
    printf("\nERROR: Tree Kernels cannot be used over sequences (positions %d "
           "or %d) \n\n",
           i, j);
    fflush(stdout);
    exit(-1);
  ...
  case -1:
    if (TKGENERALITY > SUBSET_TREE_KERNEL) {
      printf(
          "\nERROR: SHALLOW SEMANTIC TK kernel (-F -1) cannot be used on trees "
          "of Generality higher than 1, i.e. the subset tree kernel \n\n",
          i, j);
      fflush(stdout);
      exit(-1);
    }
  ...
  case 0:
    if (TKGENERALITY > SUBSET_TREE_KERNEL) {
      printf("\nERROR: ST kernel (-F 0) cannot be used on trees of Generality "
             "higher than 1, i.e. the subset tree kernel \n\n",
             i, j);
      fflush(stdout);
      exit(-1);
    }
    SIGMA = 0;
    return evaluate_SST_ST(intersect,
                           n_pairs); // ST kernel NISP2001 (Wisnathan and Smola)

  case 1:
    if (TKGENERALITY > SUBSET_TREE_KERNEL) {
      printf("\nERROR: SST kernel (-F 1) cannot be used on trees of Generality "
             "higher than 1, i.e. the subset tree kernel \n\n",
             i, j);
      fflush(stdout);
      exit(-1);
    ...
    SIGMA = 1;
    return evaluate_SST_ST(intersect,
                           n_pairs); // SST kernel (Collins and Duffy, 2002)

  case 2:
    return evaluate_GSST(intersect,
                         n_pairs); // SST kenel + bow kernel on leaves,
...
double tree_kernel(KERNEL_PARM *kernel_parm, DOC *a, DOC *b, int i, int j) {
  if (a->num_of_trees == 0 || b->num_of_trees == 0)
    ...
    double k =
        tree_kernel_not_norm(kernel_parm, a, b, i, j) /
...
/* calculate the kernel function */
{
  ...
  case 2: /* radial basis function */
    return (exp(-kernel_parm->rbf_gamma *
                (a->vectors[i]->twonorm_sq -
                 2 * sprod_ss(a->vectors[i]->words, b->vectors[j]->words) +
                 b->vectors[i]->twonorm_sq)));
  case 3: /* sigmoid neural net */
    return (tanh(kernel_parm->coef_lin *
  ...
  case 6:
    return string_kernel(kernel_parm, a, b, i, j);
  default:
    printf("Error: The kernel function to be combined with the Tree Kernel is "
           "unknown\n");
    fflush(stdout);
    exit(1);
  }
...
double basic_kernel(KERNEL_PARM *kernel_parm, DOC *a, DOC *b, int i, int j) {
  if (a->num_of_vectors == 0 || b->num_of_vectors == 0)
    ...
    if (a->num_of_vectors <= i)
      printf("\n     :position %d of the vector set of the first istance (line "
             "%d)\n\n",
             i, a->docnum + 1);
    if (a->num_of_vectors <= j)
      printf("\n     :position %d of the vector set of the second instance "
             "(line %d)\n\n",
             j, b->docnum + 1);
    fflush(stdout);
    exit(-1);
    ...
    return basic_kernel_not_norm(kernel_parm, a, b, i, j) /
...
void evaluateNorma(KERNEL_PARM *kernel_parm, DOC *d) {
  ...
  short kernel_type_tmp =
      kernel_parm->first_kernel; // save parameters from command line
  ...
  for (i = 0; i < d->num_of_trees; i++) {
    ...
    if (d->num_of_trees > i && d->forest_vec[i]->root != NULL) {
      ...
      printf("tree: <");
      writeTreeString(d->forest_vec[i]->root);printf(">");fflush(stdout);
      ...
      if (PARAM_VECT == 1) {
        if (tree_kernel_params[i].kernel_type != NOKERNEL) {
          if (tree_kernel_params[i].normalization == 1) {
            ...
            d->forest_vec[i]->twonorm_PT =
                tree_kernel_not_norm(kernel_parm, d, d, i, i);
        ...
        if (k != 0 && (kernel_parm->normalization == 1 ||
    ...
    kernel_parm->first_kernel =
        kernel_type_tmp; // re-set command line parameters
    LAMBDA = lambda_tmp;
    MU = mu_tmp;
  ...
  for (i = 0; i < d->num_of_vectors; i++) {
    ...
    if (d->num_of_vectors > 0 && d->vectors[i] != NULL) {
      d->vectors[i]->twonorm_STD =
          1; // basic-kernel normalizes the standard kernels
             // this also avoids to check for norm == 0
      k = basic_kernel_not_norm(kernel_parm, d, d, i, i);
      d->vectors[i]->twonorm_sq =
          sprod_ss(d->vectors[i]->words, d->vectors[i]->words);
      if (k != 0 &&
          (kernel_parm->normalization == 2 || kernel_parm->normalization == 3))
...
double two_set_trees(KERNEL_PARM *kernel_parm, DOC *a, DOC *b, int memberA,
                     int memberB) { // all_vs_all vectorial kernel
  ...
  for (i = 0; i < a->num_of_trees / 2 && i < b->num_of_trees / 2; i++)
    ...
    if (a->forest_vec[i + startA] != NULL &&
        b->forest_vec[startB + i] != NULL) {
...
double sequence_tree_kernel(KERNEL_PARM *kernel_parm, DOC *a, DOC *b) {
  ...
  for (i = 0; i < a->num_of_trees && i < b->num_of_trees; i++) {
    ...
    //    a->forest_vec[i]->listSize,b->forest_vec[i]->listSize); printf("doc
    //    IDs :%ld %ld",a->docnum,b->docnum); printf("\ntree 1: ");
    //    writeTreeString(a->forest_vec[i]->root); printf("\ntree 2: ");
    //    writeTreeString(b->forest_vec[i]->root);
    //
    k += tree_kernel(kernel_parm, a, b, i, i);

    //   if(k>0) printf("Kernel :%0.20f norm
    //   %f\n",k,sqrt(a->forest_vec[i]->twonorm_PT *
    //   b->forest_vec[i]->twonorm_PT));
  }
  //   printf("Kernel :%f \n",k);
...
double AVA_tree_kernel(KERNEL_PARM *kernel_parm, DOC *a,
...
// ranking algorithm based on only trees. It can be used for parse-tree
// re-ranking

double tree_kernel_ranking(KERNEL_PARM *kernel_parm, DOC *a,
                           DOC *b) { // all_vs_all_tree_kernel
  ...
  if (k < 0) {
    printf("\nWarning: negative preference kernel \n");
    fflush(stdout);
  }
...
// ranking algorithm based on only vectors. For example, it can be used for
// ranking documents wrt a query
...
// ranking algorithm based on tree forests. In this case the ranked objetcs are
...
double vector_sequence_ranking(KERNEL_PARM *kernel_parm, DOC *a, DOC *b) {
  ...
  k += two_vector_set(kernel_parm, a, b, 0,
                      0); // ranking with sequences of vectors
...
/* this means that there are n/2 vectors for the first pair and n/2 for the
 * second pair */
...
// when will be available more kernel types, remeber to define a first_kernel
// option (e.g. -F)
...
double advanced_kernels(KERNEL_PARM *kernel_parm, DOC *a, DOC *b) {
  ...
  /* TEST
          tmp = (k1*k2);
          printf("K1 %f and K2= %f NORMA= %f norma.a= %f  norma.b=
     %f\n",k1,k2,norma,a->twonorm_sq,b->twonorm_sq); printf("\nKernel
     Evaluation: %1.20f\n", tmp);
  */
...
// kernel for for question and answer classification [Moschitti et al., ACL
// 2007] BOW, POS, PAS, PT, PASN

double Question_Answer_Classification(KERNEL_PARM *kernel_parm, DOC *a,
                                      DOC *b) { // all_vs_all_tree_kernel

  int i, j;
  double k = 0, k0, k1, k2, k3, k4, k5, k15, k16, k17, k20, k21, k22, Kall_all;
  ...
  for (i = 0; i < 20; i++) {
    if (tree_kernel_params[i].kernel_type != NOKERNEL) {
      if (tree_kernel_params[i].kernel_type == END_OF_TREE_KERNELS) {
        printf("\nERROR: the parameters found in the parameter file/VECTOR are "
               "lower than");
        printf("\n       those required by the trees in the forest (only %d "
               "trees are fulfilled)\n\n",
               i);
        exit(-1);
      }
  ...
  // A PAS 1, 2 and 3
  //	k20 = tree_kernel(kernel_parm, a,b,20,20); k21 =
  // tree_kernel(kernel_parm, a,b,21,21); k22 = tree_kernel(kernel_parm,
  // a,b,22,22);

  // KALL
  kernel_parm->first_kernel = tree_kernel_params[20].kernel_type;
  LAMBDA = tree_kernel_params[20].lambda;
  LAMBDA2 = LAMBDA * LAMBDA;
  MU = tree_kernel_params[i].mu;
  TKGENERALITY = tree_kernel_params[20].TKGENERALITY;
  ...
  k = k + Kall_all;
  //	k=Kall_all*kernel_parm->tree_constant+k0+k1+k16+k15;	  // I removed
  // the tree constant default was .3 ...how much was before did you use -T 1
  //?... I hope 1
...
double Jeopardy_QC(KERNEL_PARM *kernel_parm, DOC *a, DOC *b) {
  ...
  if (kernel_parm->combination_type == '+' &&
      (a->vectors != NULL && b->vectors != NULL))
    k += basic_kernel(kernel_parm, a, b, 0, 0);

  return k;
}

// Question Answer re-ranking Jeopardy
...
double Jeopardy_QAR(KERNEL_PARM *kernel_parm, DOC *a, DOC *b) {
  ...
  for (i = 0; i < 3 && i < a->num_of_trees && i < b->num_of_trees; i++) {
    if (tree_kernel_params[i].kernel_type != NOKERNEL) {
      if (tree_kernel_params[i].kernel_type == END_OF_TREE_KERNELS) {
        printf("\nERROR: the parameter found in the parameter file/VECTOR are "
               "lower than");
        printf("\n       those required by the trees in the forest (only %d "
               "trees are fulfilled)\n\n",
               i);
  ...
  max = 0;
  for (i = 3; i < a->num_of_trees; i++)
    for (j = 3; j < b->num_of_trees; j++)
      // if
      // (tree_kernel_params[i].kernel_type!=NOKERNEL||tree_kernel_params[j].kernel_type!=NOKERNEL)
      // if((temp=tree_kernel(kernel_parm, a,b,i,j))>max) max = temp;
      max += tree_kernel(kernel_parm, a, b, i, j);
...
//	if (b->num_of_trees > 5 && a->num_of_trees>5){ // 2 normal treee, 2
placeholdered tree, and all the other assigment
//		max = 0;
                if (b->num_of_trees > 5  && a->num_of_trees > 5){
                        if(b->num_of_trees >  a->num_of_trees) {
                                for(i=2;i+1<b->num_of_trees ;i+=2){
                                        k= tree_kernel(kernel_parm, a, b, 2, i);
                                        k+= tree_kernel(kernel_parm, a, b, 3,
i+1); if(max<k)max=k;
                                }
                        }  else{
                for(i=2;i+1<a->num_of_trees ;i+=2){
                                        k= tree_kernel(kernel_parm, a, b, i, 2);
                                        k+= tree_kernel(kernel_parm, a, b, i+1,
3); if(max<k)max=k;
                                }
                        }
                }
...
b->vectors!=NULL)) return basic_kernel(kernel_parm, a, b, 0, 0)+
                kernel_parm->tree_constant*max;
        else return max;
}
*/
...
double ACL2008_Entailment_kernel(KERNEL_PARM *kernel_parm, DOC *a, DOC *b) {
  ...
  if (PARAM_VECT == 0) {
    printf("\n\nERROR!!: This kernel requires external parameter file (option "
           "-U 1)\n\n");
    exit(-1);
  }
  ...
  if (kernel_parm->combination_type == '+' &&
    ...
    double KK;
    int tmp1, tmp2;
    KK = basic_kernel(kernel_parm, a, b, 0, 0) +
         kernel_parm->tree_constant * max;
...
double SRL_re_ranking_CoNLL2006(KERNEL_PARM *kernel_parm, DOC *a,
                                DOC *b) { // all_vs_all_tree_kernel
  ...
  if (kernel_parm->kernel_type == 13 || kernel_parm->kernel_type == 12) {
    k2 += two_vector_set(kernel_parm, a, b, 0, 0);
    k2 += two_vector_set(kernel_parm, a, b, 1, 1);
    k2 -= two_vector_set(kernel_parm, a, b, 0, 1);
    k2 -= two_vector_set(kernel_parm, a, b, 1, 0);
  }
...
// Kernel for re-ranking predicate argument structures, [Moschitti, ACL 2009 -
// attempt]
...
/* calculate the kernel function */
{
  ...
  case 16:
    ...
    SK3 = tree_kernel(kernel_parm, a, b, 3, 3) +
          tree_kernel(kernel_parm, a, b, 4, 4);
    //  SK5= tree_kernel(kernel_parm, a, b, 5, 5)+tree_kernel(kernel_parm, a, b,
    //  6, 6); SK7= tree_kernel(kernel_parm, a, b, 7,
    //  7)+tree_kernel(kernel_parm, a, b, 8, 8);
    kernel_parm->first_kernel = 1;
    TK0 = tree_kernel(kernel_parm, a, b, 0,
                      0); // Evaluate tree kernel between the two i-th trees.
    //  TK1 = tree_kernel(kernel_parm, a, b, 1, 1)+tree_kernel(kernel_parm, a,
    //  b, 2, 2);
    Poly = basic_kernel(kernel_parm, a, b, 0,
                        0); // Compute standard kernel (selected according to
                            // the "second_kernel" parameter).
  ...
  case 17:
    ...
    SK3 = tree_kernel(kernel_parm, a, b, 3, 3) +
          tree_kernel(kernel_parm, a, b, 4, 4);
    // SK5= tree_kernel(kernel_parm, a, b, 5, 5)+tree_kernel(kernel_parm, a, b,
    // 6, 6); SK7= tree_kernel(kernel_parm, a, b, 7, 7)+tree_kernel(kernel_parm,
    // a, b, 8, 8);
    kernel_parm->first_kernel = 1;
    TK0 = tree_kernel(kernel_parm, a, b, 0,
                      0); // Evaluate tree kernel between the two i-th trees.
    TK1 = tree_kernel(kernel_parm, a, b, 1, 1) +
          tree_kernel(kernel_parm, a, b, 2, 2);
    Poly = basic_kernel(kernel_parm, a, b, 0,
                        0); // Compute standard kernel (selected according to
                            // the "second_kernel" parameter).
  ...
  case 18:
    ...
    // SK5= tree_kernel(kernel_parm, a, b, 5, 5)+tree_kernel(kernel_parm, a, b,
    // 6, 6);
    SK7 = tree_kernel(kernel_parm, a, b, 7, 7) +
          tree_kernel(kernel_parm, a, b, 8, 8);
    kernel_parm->first_kernel = 1;
    TK0 = tree_kernel(kernel_parm, a, b, 0,
                      0); // Evaluate tree kernel between the two i-th trees.
    TK1 = tree_kernel(kernel_parm, a, b, 1, 1) +
  ...
  case 19:
    ...
    SK3 = tree_kernel(kernel_parm, a, b, 3, 3) +
          tree_kernel(kernel_parm, a, b, 4, 4);
    SK5 = tree_kernel(kernel_parm, a, b, 5, 5) +
          tree_kernel(kernel_parm, a, b, 6, 6);
    SK7 = tree_kernel(kernel_parm, a, b, 7, 7) +
          tree_kernel(kernel_parm, a, b, 8, 8);
    kernel_parm->first_kernel = 1;
    TK0 = tree_kernel(kernel_parm, a, b, 0,
  ...
  case 21:
    kernel_parm->first_kernel = 6;
    //  SK3= tree_kernel(kernel_parm, a, b, 3, 3)+tree_kernel(kernel_parm, a, b,
    //  4, 4);
    SK5 = tree_kernel(kernel_parm, a, b, 5, 5) +
          tree_kernel(kernel_parm, a, b, 6, 6);
    //  SK7= tree_kernel(kernel_parm, a, b, 7, 7)+tree_kernel(kernel_parm, a, b,
    //  8, 8);
    kernel_parm->first_kernel = 1;
    TK0 = tree_kernel(kernel_parm, a, b, 0,
                      0); // Evaluate tree kernel between the two i-th trees.
  ...
  case 25:
    ...
    SK9 = tree_kernel(kernel_parm, a, a, 11, 12) *
          tree_kernel(kernel_parm, b, b, 11, 12);
    // kernel_parm->first_kernel=1;
    // TK0 = tree_kernel(kernel_parm, a, b, 0, 0); // Evaluate tree kernel
    // between the two i-th trees.
    //  TK1 = tree_kernel(kernel_parm, a, b, 1, 1)+tree_kernel(kernel_parm, a,
    //  b, 2, 2);
    Poly = basic_kernel(kernel_parm, a, b, 0,
                        0);  // Compute standard kernel (selected according to
