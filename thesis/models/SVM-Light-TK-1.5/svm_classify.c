...
int main(int argc, char *argv[]) {
  ...
  nol_ll(docfile, &max_docs, &max_words_doc,
         &lld); /* scan size of input file */
  max_words_doc += 2;
  lld += 2;
  nol_ll(modelfile, &max_sv, &max_words_sv,
         &llsv); /* scan size of model file */
  max_words_sv += 2;
  llsv += 2;
  ...
  while ((!feof(docfl)) && fgets(line, (int)lld, docfl)) {
    ...
    totdoc++;
    if (model.kernel_parm.kernel_type == 0) { /* linear kernel */
      for (j = 0; (doc.vectors[0]->words[j]).wnum != 0;
           j++) { /* Check if feature numbers   */
        if ((doc.vectors[0]->words[j]).wnum >
            model.totwords)                    /* are not larger than in     */
  ...
  if ((!no_accuracy) && (verbosity >= 1)) {
    float P, R;
    printf("Accuracy on test set: %.2f%% (%ld correct, %ld incorrect, %ld "
           "total)\n",
...
void print_help(void) {
  ...
  copyright_notice();
  printf("   usage: svm_classify [options] example_file model_file "
         "output_file\n\n");
  printf("options: -h         -> this help\n");
  printf("         -v [0..3]  -> verbosity level (default 2)\n");
  printf("         -f [0,1]   -> 0: old output format of V1.0\n");
  printf("                    -> 1: output the value of decision function "
         "(default)\n\n");
}
