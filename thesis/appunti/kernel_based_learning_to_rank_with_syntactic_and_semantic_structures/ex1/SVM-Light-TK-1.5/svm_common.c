...
{
  return (
      (double)(sprod_ns(model->lin_weights, ex->vectors[0]->words) - model->b));
}
...
/* calculate the kernel function */
{
  ...
  case 2: /* radial basis function */
    return ((CFLOAT)exp(-kernel_parm->rbf_gamma *
                        (a->twonorm_sq - 2 * sprod(a, b) + b->twonorm_sq)));
  case 3: /* sigmoid neural net */
    return ((CFLOAT)tanh(kernel_parm->coef_lin * sprod(a, b) +
                         kernel_parm->coef_const));
  ...
  case 6: /* Tree kernels on u*/
    return tree_kernel(kernel_parm, a, b, atoi(kernel_parm->custom),
                       atoi(kernel_parm->custom));

  case 11: /* Re-ranking of predicate argument structures only trees, i.e.
              re-ranking using trees */
  ...
  case 17:
    return (CFLOAT)ACL2008(kernel_parm, a, b);
    /* Question Classification for Jeopardy */
  case 18:
    return (CFLOAT)Jeopardy_QC(kernel_parm, a, b);
    /* Question Answer re-ranking for Jeopardy */
  case 19:
    return (CFLOAT)Jeopardy_QAR(kernel_parm, a, b);

  case 30: /* Classification of Entailment pairs */
    return Question_Answer_Classification(kernel_parm, a, b);

  case 51: /* Re-ranking of predicate argument structures only trees, i.e.
              re-ranking using trees */
  ...
  case 53: /* Re-ranking of predicate argument structures only vectors */
    return ((CFLOAT)SRL_re_ranking_ACL2009(kernel_parm, a, b));
  case 100: /* Use only the tree whose index is specified in the custom
               parameter */
    return ((CFLOAT)tree_kernel(kernel_parm, a, b, atoi(kernel_parm->custom),
                                atoi(kernel_parm->custom)));
  ...
  default: /* Advanced vectorial kernels*/
    ...
    /*if (b->num_of_trees > 0 && a->num_of_trees>0) {
          printf("\n tree 1: <"); writeTreeString(a->forest_vec[0]->root);
                           printf(">\n tree 2: <");
     writeTreeString(b->forest_vec[0]->root);printf(">\n"); printf("Kernel
     :%1.20lf \n",k1);
     }*/
  }
}
...
double sprod(DOC *a, DOC *b) { // compatibility with standard svm-light
  if (a->num_of_vectors > 0 && b->num_of_vectors > 0) {
    if (a->vectors[0] == NULL || b->vectors[0] == NULL) {
      printf("ERROR: first vector not defined (with a traditional kernel it "
             "must be defined)\n");
      exit(-1);
    } else
...
double sprod_i(DOC *a, DOC *b, int i,
               int j) { // compatibility with standard svm-light
  if (a->num_of_vectors > 0 && b->num_of_vectors > 0) {
...
/* compute weight vector in linear case and add to model */
{
  ...
  model->lin_weights =
      (double *)my_malloc(sizeof(double) * (model->totwords + 1));
...
void read_input_tree_kernel_param() {
  ...
  if (params != NULL) {
    ...
    fflush(stdin);
    do {
      fscanf(params, "%ld,", &tree_kernel_params[i].kernel_type);
      fscanf(params, "%ld,", &tree_kernel_params[i].TKGENERALITY);
      ...
      /*           printf("tree number %d:
         %d,",i,tree_kernel_params[i].kernel_type);
      ...
      */
      i++;
             ...
             tree_kernel_params[i].kernel_type != END_OF_TREE_KERNELS);
    fclose(params);
    for (j = 0; j < i; j++) {
...
void read_model(char *modelfile, MODEL *model, long max_words_doc,
                long int ll) {
  FILE *modelfl;
  int i;
  int pos;
  ...
  if (PARAM_VECT == 1)
    read_input_tree_kernel_param(); // if there is the file tree_kernel.param
                                    // load paramters
  ...
  for (i = 1; i < model->sv_num; i++) {
    ...
    while (!isspace((int)line[pos]) && line[pos] != 0)
      pos++; // go after alpha
    ...
    read_tree_forest(line, model->supvec[i],
                     &pos); // read the tree forest: set PARAM_VECT to 0 if less
                            // paramaters than vector entries
    PARAM_VECT =
        model->kernel_parm.tree_kernel_params; // restore parameter file flag
    ...
    //  if vectors exists
    if (line[pos] != 0)
      read_vector_set(line + pos, model->supvec[i], max_words_doc,
                      &fake_totwords); // read the set of vectors
    else {
      model->supvec[i]->vectors = NULL;
      model->supvec[i]->num_of_vectors = 0;
    }
...
void read_documents(char *docfile, DOC *docs, double *label,
                    long int max_words_doc, long int ll, long int *totwords,
                    long int *totdoc, KERNEL_PARM *kernel_parm) {
  char *line;
  DOC doc;
  ...
  while ((!feof(docfl)) && fgets(line, (int)ll, docfl)) {
    ...
    if (!parse_document(line, &doc, &doc_label, totwords, max_words_doc,
                        kernel_parm)) {
    ...
    // less than 5 basic kernels and greater than 50 only vectors (to save
    // memory)
    if (kernel_parm->kernel_type <
        4) {            // from 0 to 3 are original kernels => no trees
...
int parse_document(char *line, DOC *doc, double *label, long int *totwords,
                   long int max_words_doc, KERNEL_PARM *kernel_parm) {
  int pos;
  ...
  if (sscanf(line + pos, "%lf", label) == EOF)
    return (0);
  // printf("LINE:%s\n\n\n",line);fflush(stdout);
  ...
  read_tree_forest(line, doc, &pos); // read the tree forest: set PARAM_VECT to
                                     // 0 if less paramaters than vector entries
  PARAM_VECT = kernel_parm->tree_kernel_params; // restore parameter file flag
  //  printf("\nNumber of trees %d",doc->num_of_trees);

  /* Look for Standard Features, pos returns the end of the last tree*/
  //  printf("LINE:%s\n",line+pos);fflush(stdout);

  while (isspace((int)line[pos]) && line[pos] != 0)
    pos++; // go to "|BV|" marker or to the first number
           /*while((!isspace((int)line[pos])) && line[pos]) pos++;*/
  if (line[pos] != 0)
    read_vector_set(line + pos, doc, max_words_doc,
                    totwords); // read the tree forest
  else {
    doc->vectors = NULL;
...
int isnan(double a) { return (_isnan(a)); }
...
while(pippo->wnum!=0){printf ("%ld:%lf
",pippo->wnum,pippo->weight);pippo++;};fflush(stdout);
pippo=b->vectors[0]->words;printf("\t");
while(pippo->wnum!=0){printf ("%ld:%lf
",pippo->wnum,pippo->weight);pippo++;};fflush(stdout); printf("  KERNEL
%lf\n",k1);
}
