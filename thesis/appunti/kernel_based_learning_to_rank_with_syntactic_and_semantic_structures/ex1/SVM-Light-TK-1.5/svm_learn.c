...
void perceptron_learn_classification(DOC *docs, double *target, long int
			      totdoc, long int totwords, 
			      LEARN_PARM *learn_parm, 
			      KERNEL_PARM *kernel_parm, 
			      KERNEL_CACHE *kernel_cache, MODEL *model, char *modelfile){
...
long runtime_start,runtime_end;
  ...
  long max_docs,max_words_sv,lld,llsv;
...
//  model->supvec[0]=&docs[0];
//  model->alpha[0]=eta; //put learning rate here
//  model->b=r_delta_sq*eta;	       
//index[0]=1;
...
if ((modelfl = fopen (modelfile, "r")) != NULL){
     printf("Model file already exists, continuing learning\n");
	 fclose(modelfl);
	 nol_ll(modelfile,&model->sv_num,&max_words_sv,&llsv); /* scan size of model file */
	 max_words_sv+=2;
	 llsv+=2;

     model->supvec = (DOC **)my_malloc(sizeof(DOC *)*(model->sv_num+totdoc+2));
	 model->alpha = (double *)my_malloc(sizeof(double)*(model->sv_num+totdoc+2));
	 model->index = (long *)my_malloc(sizeof(long)*(model->sv_num+totdoc+2));
	 ...
	 model->alpha[0]=0;
	 for(i=model->sv_num;i<model->sv_num+totdoc+1;i++) model->alpha[i] = 0;
    }
...
misclassified=0;
  for(i=0;i<totdoc;i++){
	  ...
	  for(j=1;j<model->sv_num;j++) {
			perceptronoutput += 
			model->alpha[j]*kernel(kernel_parm, &docs[i], model->supvec[j]);
	  }

	  if(target[i]*(perceptronoutput - model->b) <= 0.0) { // < 0
		if(model->supvec[model->sv_num]==NULL) model->supvec[model->sv_num] = &docs[i];
		model->alpha[model->sv_num] += target[i];
		model->b -= eta*target[i]*r_delta_sq; // this is r_delta of the current docs not of the first round
		misclassified++;
		model->sv_num++;
	  }
	  
	  if(!(i%100)){
			printf("\r Learning ETA: %2.2f%% - Elapsed Time (in Sec):  %.2f - SV: %d",100*i/(float)totdoc,
				  (get_runtime()-runtime_start)/100.0, model->sv_num);fflush(stdout);
...
/*
	  ...
	  if(!(i%100)){
	        time=(get_runtime()-runtime_start)/100.0;
	        days = time/3600/24
			min = (time/60);
			second=time % 60;
			hours = second/60;
			
			printf("\r Learning ETA: %2.2f%% - Elapsed Time (in Sec.):  %.2f - Estimated Rmaining Time: d:%d, h:%d, m:%d, s:%d SV: %d",
			100*i/(float)totdoc,
			time, model->sv_num);fflush(stdout);
...
misclassified=0;
  for(i=0;i<totdoc;i++){
	  ...
	  for(j=1;j<model->sv_num;j++) {
	        ...
	        if(alpha[model->supvec[j]->docnum])perceptronoutput += 
			alpha[model->supvec[j]->docnum]*target[model->supvec[j]->docnum]*
			kernel(kernel_parm, &docs[i], model->supvec[j]);
	  }
	  if(target[i]*(perceptronoutput + model->b) <= 0.0) misclassified++;
  }*/
	
   printf("Misclassified: %d - SV:%d\n",misclassified,model->sv_num-1);
}
...
//      example_length=estimate_sphere(model,kernel_parm); 
      ...
      fprintf(stdout,"Threshold: b=%.5f\n",model->b);
//      fprintf(stdout,"Estimated VCdim of classifier: VCdim<=%.5f\n",
//	      estimate_margin_vcdim(model,model_length,example_length,
//				    kernel_parm));
	}
	
//	Remeber to do a free of model and some other data
}



void yyyperceptron_learn_classification(DOC *docs, double *target, long int
			      totdoc, long int totwords, 
			      LEARN_PARM *learn_parm, 
			      KERNEL_PARM *kernel_parm, 
...
TIMING timing_profile;
  ...
  model->alpha = (double *)my_malloc(sizeof(double)*(totdoc+2));
  model->index = (long *)my_malloc(sizeof(long)*(totdoc+2));
  index = (long *)my_malloc(sizeof(long)*(totdoc+2));
  alpha = (double *)my_malloc(sizeof(double)*(totdoc+2));
  cache = (double **)my_malloc(sizeof(double)*(totdoc+2));
...
for(i=0;i<totdoc;i++)alpha[i]=index[i]=0;
eta= 1;
one_row=(double *)my_malloc(sizeof(double)*(totdoc+2)); 
model->b = 0;
  ...
  for(i=0;i<totdoc;i++){
	  ...
	  if(!index[i]){
		 for(j=1;j<model->sv_num;j++) {    
			perceptronoutput += 
			alpha[model->supvec[j]->docnum]*target[model->supvec[j]->docnum]*
			(one_row[model->supvec[j]->docnum]=kernel(kernel_parm, &docs[i], model->supvec[j]));
	     }
	   }  else
	       for(j=1;j<model->sv_num;j++) {    
			perceptronoutput += 
			alpha[model->supvec[j]->docnum]*target[model->supvec[j]->docnum]*cache[model->supvec[j]->docnum][i];
	       }
		   
	  if(target[i]*(perceptronoutput + model->b) <= 0.0) {
		if(!index[i]) {
			model->supvec[model->sv_num++]=&docs[i]; //complete the row
			 for(j=0;j<totdoc;j++)if(!index[j]) one_row[j]=kernel(kernel_parm, &docs[i], &docs[j]);
			 index[i]=1;
			 cache[i]=one_row;
//			for(j=1;j<model->sv_num;j++)
//			printf("kernel(%d,%d) %lf\n",i,model->supvec[j]->docnum,cache[i][model->supvec[j]->docnum]);

			one_row=(double *)my_malloc(sizeof(double)*(totdoc+2)); 
			}
		alpha[i] += eta;
		model->b += eta*target[i]*r_delta_sq;
//		misclassified++;
	  }
 ...
 /*  misclassified=0;
  for(i=0;i<totdoc;i++){
   ...
   printf("Iteration %d - Missclassified: %d\n",k,misclassified);
}

for(j=1;j<model->sv_num;j++) model->alpha[j]=target[model->supvec[j]->docnum]*alpha[model->supvec[j]->docnum];
model->b=-model->b;
...
void batch_perceptron_learn_classification(DOC *docs, double *target, long int
			      totdoc, long int totwords, 
			      LEARN_PARM *learn_parm, 
			      KERNEL_PARM *kernel_parm, 
			      int kernel_cache_size, MODEL *model){
     /* docs:        Training vectors (x-part) */
     /* class:       Training labels (y-part, zero if test example for
                     transduction) */
     /* totdoc:      Number of examples in docs/label */
...
long runtime_start,runtime_end,runtime_inter,
		*index;
...
TIMING timing_profile;
  ...
  runtime_inter=runtime_start=get_runtime();
  timing_profile.time_kernel=0;
  timing_profile.time_opti=0;
  timing_profile.time_shrink=0;
  timing_profile.time_update=0;
  ...
  index = (long *)my_malloc(sizeof(long)*(totdoc+2));
  alpha = (double *)my_malloc(sizeof(double)*(totdoc+2));
  cache = (float **)my_malloc(sizeof(float *)*(totdoc+2));
...
for(i=0;i<totdoc+1;i++){
	alpha[i]=index[i]=0;
	cache[i]=NULL;//(double *)my_malloc(sizeof(double)*(totdoc+2));	 for(l=0;l<totdoc+1;l++)cache[i][l]=-1.0;
}

eta= 1;
model->b = 0;
misclassified=1;
occupancy=((totdoc+2)*sizeof(float))/1024.0/1024.0;

printf("Kernel Cash: %ld Mbs, Memory for one SV:%f Mbs\n\n",kernel_cache_size,occupancy);

if(learn_parm->svm_iter_to_shrink==0)learn_parm->svm_iter_to_shrink=999999;

for(k=0;k<learn_parm->svm_iter_to_shrink && misclassified ;k++) // use the value of shrinking param
{
  misclassified=a=b=c=d=0;
  
  for(i=0;i<totdoc;i++){
	  perceptronoutput = 0;current_cache_dim=(model->sv_num+1)*occupancy;
	  for(j=1;j<model->sv_num;j++) {  
		   if(cache[model->supvec[j]->docnum]!=NULL){
		        if (cache[model->supvec[j]->docnum][i]>=0) val=cache[model->supvec[j]->docnum][i];
					else val = cache[model->supvec[j]->docnum][i] = kernel(kernel_parm, &docs[i], model->supvec[j]);
			}
				 else val = kernel(kernel_parm, &docs[i], model->supvec[j]);
		    	 
			perceptronoutput += alpha[model->supvec[j]->docnum]*target[model->supvec[j]->docnum]*val;
	  }
	  
	  if(target[i]*(perceptronoutput + model->b) <= 0.0) {
		
		if(!index[i]) {
			model->supvec[model->sv_num++]=&docs[i]; //reserve memory for the SV cash
			if(kernel_cache_size > current_cache_dim){
		            cache[i]=(float *)my_malloc(sizeof(float)*(totdoc+2)); 
			        for(l=0;l<totdoc+1;l++)cache[i][l]=-1.0;
			  }
		 index[i]=1;
		}
		...
		model->b += eta*target[i]*r_delta_sq;
		misclassified++;
		if(target[i]==1) c++; else b++;
	  } 
	else if(target[i]==1)a++; else d++;

   if(!(i%100)){printf(".");fflush(stdout);}

   }	  
	  
   printf("\nIteration %d - Misclassified: %d - Support Vectors: %d\n",k,misclassified,model->sv_num-1);
   if(a!=0) printf("Precision: %f - Recall: %f - F1: %f\n",a/((float)a+b),a/((float)a+c),
												  2/(((float)a+b)/a+((float)a+c)/a));
   printf("One Cycle Runtime in cpu-seconds: %.2f\n",(get_runtime()-runtime_inter)/100.0);
   runtime_inter=get_runtime();

   }

   printf("\n\n\nFinished: Number of Support Vectors: %d\n",model->sv_num-1);

// Measure Perf
  if((occupancy*totdoc)/kernel_cache_size<5){ // too much instances it would be too slow
	a=b=c=d=0;
...
model->b=-model->b;
   ...
   printf("\n");  
	...
	(100.0*timing_profile.time_shrink)/(float)(runtime_end-runtime_start),
      ...
      printf("Total Learning Runtime in cpu-seconds: %.2f\n",
	     (runtime_end-runtime_start)/100.0);
    }
...
/*	printf("parse tree:");
        writeTreeString(docs[i].root);
	printf("\ndoc nunm, a,label,lin:%d, %f, %d, %f\n",i, a[i],label[i],lin[i]);
*/
      }
     ...
     /* model:       Returns learning result (assumed empty before called) */
{
  ...
  for(i=0;i<totdoc;i++) {
	...
	if(rankvalue[i] > rankvalue[j]) {
	  docdiff[k].vectors[0]->words=sub_ss(docs[i].vectors[0]->words,docs[j].vectors[0]->words);
	  docdiff[k].vectors[0]->twonorm_sq=sprod_ss(docdiff[k].vectors[0]->words,docdiff[k].vectors[0]->words);
	  docdiff[k].docnum=k;
...
// to be modified 
  ...
  if(kernel_parm->kernel_type==0) { /* special linear case */
    ...
    for(ii=0;(i=working2dnum[ii])>=0;ii++) {
      if(a[i] != a_old[i]) {
	add_vector_ns(weights,docs[i].vectors[0]->words,((a[i]-a_old[i])*(double)label[i]));
      }
    }
    for(jj=0;(j=active2dnum[jj])>=0;jj++) {
      lin[j]+=sprod_ns(weights,docs[j].vectors[0]->words);
    }
  }
  else {                            /* general case */
    for(jj=0;(i=working2dnum[jj])>=0;jj++) {
     ...
     /* Computes lin for those variables from scratch. */
{
  ...
  if(kernel_parm->kernel_type == LINEAR) { /* special linear case */
    ...
    for(i=0;i<totdoc;i++) {
      if(a[i] != a_old[i]) {
	add_vector_ns(weights,docs[i].vectors[0]->words,
	...
	lin[i]=shrink_state->last_lin[i]+sprod_ns(weights,docs[i].vectors[0]->words);
      }
	...
	computing a better bound using a greedy optimzation strategy. */
{
  ...
  for(d=0;d<learn_parm->xa_depth;d++) {
	...
	val=init_val_sq;
	if(kernel_parm->kernel_type == LINEAR) 
	  val+=docs[sv2dnum[i]].vectors[0]->twonorm_sq;
	else
...
{                         /* length of the longest support vector. This is */
  ...
  nulldoc.forest_vec=NULL;//&nulltree; 
  nulldoc.num_of_trees=0;

  nulldoc.vectors=NULL;
  nulldoc.num_of_vectors=0;
...
//  strcpy(nullword.sName="NULL");
 // nullword.iNoOfChildren=0;
...
//    printf("\ntree 1: "); writeTreeString(docs[i].root);printf("\n");
//    printf("twonorm:%f \n",docs[i].twonorm_sq);
    avgxlen+=sqrt(kernel(kernel_parm,&(docs[i]),&(docs[i]))
		  -2*kernel(kernel_parm,&(docs[i]),&nulldoc)
		  +kernel(kernel_parm,&nulldoc,&nulldoc));
// printf("%d) x*x:%f --- x*null:%f null*null:%f\n",i,kernel(kernel_parm,&(docs[i]),&(docs[i])),
//		  -2*kernel(kernel_parm,&(docs[i]),&nulldoc),
//		  +kernel(kernel_parm,&nulldoc,&nulldoc));
  }
