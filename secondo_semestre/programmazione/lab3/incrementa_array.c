...
// eseguo un ciclo for per traslare tutti gli elementi successivi a A[pos] di una posizione a destra	
	while (--length > pos) {
		*(A + length) = *(A + length - 1);
	}
	
// inserisco l'elemento richiesto nella posizione richiesta
	*(A + pos) = x;		
}
