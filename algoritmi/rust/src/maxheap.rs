...
where
    T: PartialOrd + Clone + AddAssign + SubAssign + Sub<Output = T>,
{
    let mut f = Dir::N;
    ...
    match f {
        Dir::N => {}
        Dir::L => {
            max_heapify(a, left(i));
        }
        Dir::R => {
            max_heapify(a, right(i));
        }
    };
}
...
where
    T: PartialOrd + Clone + AddAssign + SubAssign + Sub<Output = T> + std::fmt::Debug,
{
    let mut i = a.len() / 2 + 1;
    loop {
        i -= 1;
