...
template <class T> std::ostream &operator<<(std::ostream &, const Albero<T> &);
...
private:
  ...
  static bool rec_search(const T &t, nodo *p) {
    if (!p)
      return false;
    return t == p->info || rec_search(t, p->left) || rec_search(t, p->mid) ||
           rec_search(t, p->right);
  }
  ...
  static std::ostream &rec_print(std::ostream &os, nodo *p) {
    if (p) {
      os << p->info << " ";
      rec_print(os, p->left);
      rec_print(os, p->mid);
...
public:
  ...
  void insert(const T &t) { root = new nodo(t, copy(root), copy(root), root); }
  bool search(const T &t) const { return rec_search(t, root); }

  bool operator==(const Albero<T> &t) const { return rec_eq(root, t.root); }
};

template <class T>
std::ostream &operator<<(std::ostream &os, const Albero<T> &a) {
  return Albero<T>::rec_print(os, a.root);
}
...
int main() {
  ...
  hello.insert(9);
  hello.insert(10);
  std::cout << (hello.search(9) ? "9 is in hello" : "9 is not in hello")
            << '\n';
  std::cout << hello << '\n';
}
