...
pub trait Matrice<E: CoefficienteMatrice>: Clone {
    ...
    fn nullo(&self, index: usize) -> bool {
        // true se il numero alla posizione index == 0
    ...
    fn riga_dominante(&mut self, riga: usize) -> E {
        let mut i = 0;
        let mut sup = E::new();
        self.estrai_riga(riga).iter_mut().for_each(|x| {
            if sup.nullo() {
                if !x.nullo() {
                    sup = *x;
                }
    ...
    fn make_colonnadominante(&mut self, riga1: usize, riga2: usize) -> E {
        ...
        //    while i< self.righe() {
        let mut j = 0;
        while self.nullo(riga1 + j) {
            // controlla se il valore attuale è nullo
            j += 1;
        }
        let sup = self.value(riga2 + j); // indica la costare da moltiplicare
        if self.nullo(riga2 + j) {
            // se il valore è nullo allora si moltiplica per 0 ed è inutile
            return sup; // serve per risolvere i sistemi
        }
        while j < self.colonne() {
            // è sottratto il sup per il coefficiente della riga superiore
    ...
    fn dominante(&self, index: usize) -> Result<usize, &str> {
        ...
        while j < self.colonne() {
            ...
            while i < self.righe() {
                if !self.nullo(i * self.colonne() + j) {
                    return Ok(i);
                }
                i += 1;
            }
    ...
    fn rango(&self) -> Result<usize, &str> {
        ...
        match mat.elm_gauss() {
            Err(_) => {
                return Err("elm_gauss: le righe e le colonne non combaciano con la dimensione della matrice");
            }
            _ => {}
        }
        // effettua l'eliminazione di gauss
        Ok((0..self.righe())
            .filter(|i| mat.dominante(*i).is_ok())
            .count())
    }
    ...
    fn risolvi_sistema(&mut self, b: &mut Self) -> Result<isize, &str> {
        ...
        if self.rango()? < self.colonne() {
            return Ok(mat.check_soluzioni(b));
        }
        // println!("{}\n{}", self, b); // so far funziona!
        else {
            ...
            for colonna in 0..tmp.colonne() {
                *tmp.mut_value(riga * tmp.colonne() + colonna) = E::new() - E::new();
                for ptr in 0..self.colonne() {
                    *tmp.mut_value(riga * tmp.colonne() + colonna) = tmp
                        .value(riga * tmp.colonne() + colonna)
                        + self.value(riga * self.colonne() + ptr)
                            * a.value(ptr * a.colonne() + colonna);
                }
            }
        }
        Ok(tmp)
