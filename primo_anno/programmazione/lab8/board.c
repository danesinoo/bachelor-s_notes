...
int effettua_mossa(char board[], int x, int y, char simbolo_giocatore) {
    /*
...
int mossa_ammissibile(char board[], int x, int y) {
     ...
     */
    if (*(board+x*BOARD_LATO+y) == CASELLA_VUOTA) return 1; // 32 in ascii è lo spazio
    return 0;
}
...
int tris_in_colonna(char board[], int num_col) {
    ...
    */
    if (board[num_col] == CASELLA_VUOTA) 
        return 0;
    for (int i=1; i<BOARD_LATO; i++)
        if (board[num_col] != board[i*BOARD_LATO+num_col])
            return 0;
    return 1;
}
...
int tris_in_diagonali(char board[]) {
    /*
    POST Restituisce
            1 se almeno una diagonale è non vuota e con tutti i simboli uguali
            0 altrimenti
    */
   if (tris_backslash(board) == 1 || tris_slash(board) ==1) return 1;
   return 0;
}

int tris_backslash(char board[]) {
    if (board[0] == CASELLA_VUOTA)
        return 0;
    for (int i = 1; i < BOARD_LATO; i++) {
        if (board[0] != board[i * BOARD_LATO + i])
            return 0;
    }
    return 1;
}

int tris_slash(char board[]) {
    if (*(board + BOARD_LATO -1) == CASELLA_VUOTA) return 0;
    for (int i= 2; i <BOARD_LATO +1; i++) {
        if (*(board + BOARD_LATO -1) != *(board + i*BOARD_LATO - i)) 
        return 0;
    }
    return 1;
}
...
int partita_terminata(char board[]) {
    if (board_vincente(board) == 1 || board_riempita(board) == 1) return 1;
    return 0;
}
